...
class ImageScrollView: UIScrollView, UIScrollViewDelegate {
    ...
    override var frame: CGRect {
        willSet{
            ...
            let sizeChanging: Bool = !CGSizeEqualToSize(frame.size, newValue.size)
            if sizeChanging { self.prepareToResize()}
            
        }
        didSet {
            // check to see if there was a resize. recover if there was one
            let sizeChanged: Bool = !CGSizeEqualToSize(frame.size, oldValue.size)
    ...
    func recoverFromResizing() {
        ...
        let boundsCenter: CGPoint = self.convertPoint(_pointToCenterAfterResize, fromView: self._imageView)
